...
class AppModel {
  ...
  static async editTask({
    tasklistId,
    taskId,
    newTaskName: newTaskName1,
    newTaskTime: newTaskName2
  }) {
    const result = await fetch(
      `http://localhost:4321/tasklists/${tasklistId}/tasks/${taskId}`,
      {
        ...
        body: JSON.stringify({ newTaskName1, newTaskName2 })
      }
    );
...
class Tasklist {
  ...
  onAddTaskButtonClick = async () => {
    const newTaskName1 = prompt('Введите ФИО пациента:');
    const newTaskName2 = prompt('Введите время в формате HH:MM');
    const newTaskName = { newTaskName1, newTaskName2 };
    

    if (!newTaskName1 || !newTaskName2) return;
  ...
  onEditTask = async (taskID) => {
    ...
    const oldTaskName = this.tasks[taskIndex].newTaskName1;
    const oldTaskTime = this.tasks[taskIndex].newTaskName2;

    const newTaskName = prompt('Введите новое ФИО пациента', oldTaskName);
    const newTaskTime = prompt('Введите новое время приёма:', oldTaskTime);

    if (!newTaskName || newTaskName === oldTaskName || !newTaskTime || newTaskTime === oldTaskTime) {
      return;
    }
  ...
  onDeleteTaskButtonClick = async (taskID) => {
    ...
    if (!confirm(`Запись '${taskName.newTaskName1}, ${taskName.newTaskName2}' будет удалена. Продолжить?`)) return;
  ...
  renderTask = ({ taskID, taskName }) => {
    ...
    span1.classList.add('tm-tasklist-task-text');
    span1.innerHTML = taskName.newTaskName1;
    task.appendChild(span1);

    const span2 = document.createElement('span');
    span2.classList.add('tm-tasklist-task-text');
    span2.innerHTML = taskName.newTaskName2;
